...
class LinearYieldPredictor:
    """多作物产量预测模型 - 线性回归版本（作为基线对比）"""

    def __init__(self, data_root='./data', crop='maize', model_type='ridge'):
        """
        初始化预测器
        crop: 'maize' 或 'wheat'
    ...
    def preprocess_data(self, use_lag_features=False, add_trend_features=True):
        ...
        if add_trend_features and meteo_agg is not None:
            ...
            # 添加交互特征（线性回归需要手动构造非线性关系）
            # 温度×降水交互
            if 'tavg' in df.columns and 'prec' in df.columns:
                df['tavg_prec_interaction'] = df['tavg'] * df['prec']
                print("✓ 添加温度×降水交互特征")

            # NDVI×温度交互
            if 'ndvi_mean' in df.columns and 'tavg' in df.columns:
                df['ndvi_tavg_interaction'] = df['ndvi_mean'] * df['tavg']
                print("✓ 添加NDVI×温度交互特征")

            # 二次项（捕捉非线性）
            for var in ['tavg', 'prec', 'ndvi_mean']:
                if var in df.columns:
                    df[f'{var}_squared'] = df[var] ** 2
            print("✓ 添加二次项特征（tavg², prec², ndvi_mean²）")
        ...
        if add_trend_features:
            interaction_candidates = ['tavg_prec_interaction', 'ndvi_tavg_interaction',
                                      'tavg_squared', 'prec_squared', 'ndvi_mean_squared']
            interaction_features = [f for f in interaction_candidates if f in df.columns]
        ...
        self.feature_cols = (base_features + trend_features + available_time_features +
                             weather_features + anomaly_features + interaction_features +
                             ndvi_features + fpar_features + soil_features + soil_moisture_features)
    ...
    def _get_model(self):
        ...
        if self.model_type == 'linear':
            ...
            return ElasticNet(alpha=0.01, l1_ratio=0.5, random_state=42, max_iter=5000)
        else:
            return Ridge(alpha=1.0, random_state=42)

    def train_time_series_cv(self, n_splits=5, min_train_years=5):
        """
        使用正确的时间序列交叉验证训练线性模型
        """
        print("\n" + "=" * 50)
        print(f"开始训练{self.crop}线性回归模型（{self.model_type}，时间序列交叉验证）...")
        print("=" * 50)
        ...
        # 输出模型系数（线性回归的可解释性优势）
        print("\n" + "=" * 50)
        print(f"模型系数 Top 15 (影响最大的特征):")
        print("=" * 50)
        coefficients = self.model.coef_
        indices = np.argsort(np.abs(coefficients))[::-1][:15]
        for i, idx in enumerate(indices):
            sign = "+" if coefficients[idx] > 0 else "-"
            print(f"  {i + 1}. {self.feature_cols[idx]}: {sign} {abs(coefficients[idx]):.6f}")
    ...
    def save_model(self, path=None):
        """保存模型"""
        if path is None:
            path = f'./models/saved_models/{self.crop}_{self.model_type}.pkl'
        ...
        print(f"\n✓ {self.crop}线性模型（{self.model_type}）已保存到: {path}")
    ...
    def load_model(self, path=None):
        ...
        if os.path.exists(path):
            ...
            print(f"✓ {self.crop}线性模型已加载: {path}")
            return True
        else:
...
if __name__ == "__main__":
    ...
    # 测试不同的线性模型类型
    model_types = ['ridge', 'linear', 'lasso', 'elasticnet']

    results = {}

    for model_type in model_types:
        print("\n" + "=" * 60)
        print(f"训练玉米产量预测模型（{model_type.upper()}回归）")
        print("=" * 60)

        maize_predictor = LinearYieldPredictor(data_root='./data', crop='maize', model_type=model_type)
        maize_predictor.load_all_data()
        maize_predictor.preprocess_data(use_lag_features=False, add_trend_features=True)
        avg_r2, avg_mae = maize_predictor.train_time_series_cv(n_splits=5)
        maize_predictor.save_model()

        results[f'maize_{model_type}'] = {'r2': avg_r2, 'mae': avg_mae}

    for model_type in model_types:
        print("\n" + "=" * 60)
        print(f"训练小麦产量预测模型（{model_type.upper()}回归）")
        print("=" * 60)

        wheat_predictor = LinearYieldPredictor(data_root='./data', crop='wheat', model_type=model_type)
        wheat_predictor.load_all_data()
        wheat_predictor.preprocess_data(use_lag_features=False, add_trend_features=True)
        avg_r2, avg_mae = wheat_predictor.train_time_series_cv(n_splits=5)
        wheat_predictor.save_model()

        results[f'wheat_{model_type}'] = {'r2': avg_r2, 'mae': avg_mae}

    # 打印汇总结果
    print("\n" + "=" * 60)
    print("线性回归模型汇总结果（与随机森林对比）")
    print("=" * 60)
    print("\n玉米 (Maize):")
    print(f"  随机森林 (RF):     R² = 0.7926, MAE = 0.4306")
    for model_type in model_types:
        r2 = results[f'maize_{model_type}']['r2']
        mae = results[f'maize_{model_type}']['mae']
        print(f"  {model_type.upper():10s}: R² = {r2:.4f}, MAE = {mae:.4f}")

    print("\n小麦 (Wheat):")
    print(f"  随机森林 (RF):     R² = 0.9013, MAE = 0.3571")
    for model_type in model_types:
        r2 = results[f'wheat_{model_type}']['r2']
        mae = results[f'wheat_{model_type}']['mae']
        print(f"  {model_type.upper():10s}: R² = {r2:.4f}, MAE = {mae:.4f}")
