...
class YieldPredictorRF:
    ...
    def train_time_series_cv(self, n_splits=5, use_lag_features=False, min_train_years=5):
        ...
        for test_year in test_years:
            ...
            # 标准化（随机森林对尺度不敏感，但为了与XGBoost对比保持一致）
    ...
    def save_model(self, path=None):
        """保存模型"""
        if path is None:
            path = f'./models/saved_models/{self.crop}_randomforest.pkl'
    ...
    def load_model(self, path=None):
        ...
        if os.path.exists(path):
            ...
            print(f"✓ {self.crop}随机森林模型已加载: {path}")
            return True
        else:
...
if __name__ == "__main__":
    ...
    # 训练玉米随机森林模型
    print("\n" + "=" * 60)
    print("训练玉米产量预测模型（随机森林 + 时间序列交叉验证）")
    print("=" * 60)
    maize_predictor = YieldPredictorRF(data_root='./data', crop='maize')
    maize_predictor.load_all_data()
    maize_predictor.preprocess_data(use_lag_features=False, add_trend_features=True)
    maize_predictor.train_time_series_cv(n_splits=5)
    maize_predictor.save_model()

    # 训练小麦随机森林模型
    print("\n" + "=" * 60)
    print("训练小麦产量预测模型（随机森林 + 时间序列交叉验证）")
    print("=" * 60)
    wheat_predictor = YieldPredictorRF(data_root='./data', crop='wheat')
